...
smallval = 0.1
thetai= (45/180.0)*PI !also change ti
...
write(filename,20) 'data/rotframe',trim(adjustl(modestring)),trim(adjustl(imagestring)),thetamax,'degs',eta,'eta', &
...
! 			Eykspacearray(n,m)=0
! 		end if
		write(2,10) xtilde, ztilde, (abs(Eykspace))**2, abs(Eykspace) !intensity - therefore squared
	end do
