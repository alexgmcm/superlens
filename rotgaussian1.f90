...
thetai= PI/4.0 !used to work out rotated frame
...
eta = 3 !go from 0.1 to 5, dimensionless parameter equal to omega*d/c
	...
	do n=0, xtildesize
		...
		do p=0, 200
			integral = integral + (EXP((-sigmatilde*(kxprimearray(p)**2)/2.0) &
			 + i*kxprimearray(p)*rotspacearray(n,m,1) )*kxprimestepfrac)
		end do 
		Eykspacearray(n,m) = (1.0/sqrt(2*PI))*sqrt(sigmatilde)*EXP(i*eta*rotspacearray(n,m,2))*integral
	end do 
end do
